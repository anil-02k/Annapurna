...
const MapComponent = (props) => {
  ...
  const [center, setCenter] = useState([78.9629, 20.5937]); // India coordinates (longitude, latitude)
  ...
  // Allow external flyTo calls from SearchBox
  const onMapLoad = (map) => {
    window.map = map;
    ...
    Object.values(objects).forEach(async(value) => {
      ...
      if (e.lngLat.lng < value.maxLat && e.lngLat.lat < value.maxLng && e.lngLat.lat > value.minLng && e.lngLat.lng > value.minLat) {
        ...
        try {
          ...
          const response = await fetch('http://localhost:3002/insight/', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              "airQuality" : "API",
              "airQualityHistory" : "API",
              "fireLatest" : "API",
              "pollenForecast" : "API",
              "pollenHistory" : "API",
              "soilLatest" : "API",
              "soilHistory" : "API",
              "waterVaporHistory" : "API",
              "waterVaporLatest" : "API",
              "weatherForecast" : "API",
              "weatherHistory" : "API",
              "weatherLatest" : "API",
              "pollenLatest" : "API",
        ...
        let waterVaporStatus = null;
        const waterVapor = insights.waterVaporLatest.data;
        if (waterVapor && waterVapor.length > 0) {
          waterVaporStatus = waterVapor[0].water_vapor + '%';
        }
