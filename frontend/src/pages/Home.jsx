...
const HomePage = () => {
  ...
  const [environmentDescription, setEnvironmentDescription] = useState(t('Click on a land plot to get your farm judged by Annapurna.'));
  const [locationScore, setLocationScore] = useState ('?');
  const [locationDescription, setLocationDescription] = useState(t('Click on a land plot to get your farm judged by Annapurna.'));
  ...
  useEffect(async () => {
    const response = await fetch(
      'http://localhost:3002/data/earth',
      {
        method: 'GET',
        headers: {
